...
| --- | --- | --- |
| Linux host | Runs the Creator 5 Klipper fork, Moonraker, and the configured web interface. The observed printer is MIPS32; host builds need compatible Python dependencies and the Klipper C helper. | [klipper-c5](https://github.com/FlashForge-C5-Modding-Group/klipper-c5) |
| Mainboard MCU | Handles the main motion and machine I/O through the GD32 port. | `klipper-c5/src/c5_mainboardgd.c` and related target code |
| eboard MCU | Drives the shared physical extruder motor and supplies the pressure-advance measurement interface. | `klipper-c5/src/c5_eboard.c` and `klippy/extras/pa_adjust.py` |
| Heaterboard MCU | Handles tool heater and temperature I/O. | `klipper-c5/src/c5_heaterboard.c` |
| Levelboard MCU | Supplies probe and levelboard-specific sensing used by calibration. | `klipper-c5` MCU and Klippy levelboard code |
